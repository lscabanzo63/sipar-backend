...
class TorresConfigIn(BaseModel):
    id_conjunto: int
    id_usuario: int
    num_torres: int
    pisos_x_torre: int
    aptos_x_piso: int
    numeracion_automatica: str  # se espera "NUMERACION_AUTOMATICA"

    @field_validator("num_torres", "pisos_x_torre", "aptos_x_piso")
    @classmethod
    def positive_int(cls, v: int):
        if v <= 0:
